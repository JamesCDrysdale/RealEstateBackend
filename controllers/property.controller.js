...
const createProperty = async (req, res) => {
    try {
        ...
        // Get user who is creating the property
        const user = await User.findOne({ email }).session(session);

        if (!user) throw new Error('User not found');

        // If there is a user - get the property photo and upload it to cloudinary
        ...
        // Add new property to user's list of properties
...
const deleteProperty = async (req, res) => {
    try {
        ...
        // Create a session so that the transaction is atomic and prevent partical deletion
        const session = await mongoose.startSession();
        session.startTransaction();
